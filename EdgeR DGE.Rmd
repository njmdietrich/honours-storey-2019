...
all = cbind(all$table, countdf[,7:9])
...
```{r}
require(ggrepel)
volcano <- ggplot(data=allproteins, aes(logFC, -log2(PValue))) + geom_point(aes(color=log2(TPM)), size=1) +
        scale_colour_viridis_c() + #geom_text_repel(data=head(all, 7), aes(label=Row.names)) 
        coord_cartesian(xlim = c(-20, 20)) + geom_hline(yintercept=6.644) + geom_vline(xintercept=1.5) + geom_vline(xintercept=-1.5) +
        coord_cartesian(ylim = c(-10, 1100))
...
```{r}
sigprot = merge(sig, allproteins, all.x=TRUE, all.y=FALSE, sort=FALSE)
sum(sigprot$logFC < 0)
sum(sigprot$logFC > 0)
mean(sigprot[sigprot$logFC < 0,]$TPM)
mean(sigprot[sigprot$logFC > 0,]$TPM)
```

Overall, almost twice as many genes are upregulated than are downregulated in adult cells.\
